...
def scrape_list(list_url,
    ...
    if (page_options == []) or (page_options == "*"):
        while True:
            page_films, page_soup = scrape_page(list_url, list_url,
                                                output_file_extension, quiet,
                                                concat)
    ...
    # If page selection was input, only go to those pages
        ...
        for p in page_options:
            ...
            try:
                page_films, page_soup = scrape_page(new_link, list_url,
                                                    output_file_extension,
                                                    quiet, concat)
                list_films.extend(page_films)
            except:
...
def extract_url_shortener(html):
    # Parse the HTML
    soup = html

    # Find the <input> element within the <div> with class "urlgroup"
    urlgroup_div = soup.find('div', class_='urlgroup')
    if urlgroup_div:
        input_element = urlgroup_div.find('input')
        if input_element and input_element.get('type') == 'text':
...
def scrape_film(film_html, not_found):
    ...
    film_dict["Film_title"] = film_soup.find("div", {
        "class": "col-17"
    }).find("h1").text

    # Try to find release year, if missing or 0 insert nan
    release_year = int(
        str(film_soup.find_all("script")).split("releaseYear: ")[1].split(",")
        [0].strip("\""))
    if release_year == 0:
    ...
    director = film_soup.find('meta', attrs={
        'name': 'twitter:data1'
    }).attrs['content']
    if director == "":
        director = not_found
    ...
    try:
        cast = [
            line.contents[0]
            for line in film_soup.find('div', attrs={
                'id': 'tab-cast'
            }).find_all('a')
        ]
    ...
    try:
        ...
        film_dict["Genres"] = [
            genres.text
            for genres in genres.find_all('a', {'class': 'text-slug'})
        ]
    except:
    ...
    try:
        film_dict["Countries"] = [
            ...
            for line in film_soup.find('div', attrs={
                'id': 'tab-details'
            }).find_all('a', href=re.compile(r'country'))
        ]
    ...
    try:
        ...
        languages = [
            ...
            for line in film_soup.find('div', attrs={
                'id': 'tab-details'
            }).find_all('a', href=re.compile(r'language'))
        ]
        film_dict["Original_language"] = languages[
            0]  # original language (always first)
        film_dict["Spoken_languages"] = list(
            sorted(set(languages),
                   key=languages.index))  # all unique spoken languages
    except:
    ...
    try:
        film_dict["Studios"] = [
            ...
            for line in film_soup.find('div', attrs={
                'id': 'tab-details'
            }).find_all('a', href=re.compile(r'studio'))
        ]
    ...
    r = requests.get(f'https://letterboxd.com/csi/film/{movie}/stats/'
                     )  # Stats page of said movie
    stats_soup = BeautifulSoup(r.content, 'lxml')

    # Get number of people that have watched the movie
    watches = stats_soup.find(
        'a', {'class': 'has-icon icon-watched icon-16 tooltip'})["title"]
    watches = re.findall(r'\d+', watches)  # Find the number from string
    film_dict["Watches"] = int(
        ''.join(watches))  # Filter out commas from large numbers

    # Get number of film appearances in lists
    list_appearances = stats_soup.find(
        'a', {'class': 'has-icon icon-list icon-16 tooltip'})["title"]
    ...
    likes = stats_soup.find(
        'a',
        {'class': 'has-icon icon-like icon-liked icon-16 tooltip'})["title"]
    ...
    url_shortener = extract_url_shortener(film_soup)
    film_dict["URL Shortner"] = url_shortener
